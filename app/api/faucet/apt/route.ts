...
function readBase(): string {
  const raw = process.env.NEXT_PUBLIC_APTOS_FAUCET_URL?.trim();
  return raw && raw.length > 0 ? raw.replace(/\/+$/, "") : "https://faucet.testnet.aptoslabs.com";
}
...
export async function POST(req: Request): Promise<Response> {
  ...
  const url = `${readBase()}/mint?amount=${AMOUNT_OCTAS}&address=${encodeURIComponent(address)}`;
  try {
